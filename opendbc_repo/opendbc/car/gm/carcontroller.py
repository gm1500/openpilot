...
class CarController(CarControllerBase):
  ...
  def accel_to_torque(self, accel: float, CS, pitch: float) -> float:
    # Convert requested vehicle acceleration to the camera ACC torque command.
    return self.CP.wheelRadius * (
      self.CP.mass * accel +
      self.CP.mass * ACCELERATION_DUE_TO_GRAVITY * sin(pitch) +
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      if self.frame % 4 == 0:
        ...
        if not CC.longActive:
          ...
          torque = self.accel_to_torque(accel, CS, 0.0)  # pitch is not exposed on this platform
          brake_accel = min((torque - self.params.BRAKE_THRESHOLD) / (self.CP.wheelRadius * self.CP.mass), 0.0)
