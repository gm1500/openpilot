...
# Was implicit 2.0 * b/T and 2.0 * c/T^2. That P-gain weaves on noisy lines.
...
def apply_lane_lock(model_output: dict[str, np.ndarray], e2e_curvature: float, v_ego: float,
  ...
  Disabled with no leftover weight is exact upstream E2E. Releases (blinker,
  lane-change, weak lines, path disagreement) ramp the blend out instead of
  snapping, which was weaving the truck.
  """
  global _lane_lock_weight, _lane_lock_lane_curvature
  global _lane_lock_has_lane_curvature, _lane_lock_full_active
  global _lane_lock_error_logged

  if not lane_policy_enabled and _lane_lock_weight <= 1e-3:
    reset_lane_lock()
  ...
  force_release = (not lane_policy_enabled) or blinkers_active
  fallback_reason = ("stock-e2e (lane toggle off)" if not lane_policy_enabled
                     else "stock-e2e fallback: blinker" if blinkers_active
                     else "stock-e2e fallback: lane data unavailable")
  ...
  try:
    if not force_release:
      ...
      if lane_change_prob > LANE_LOCK_MAX_LANE_CHANGE_PROB:
        force_release = True
        fallback_reason = "stock-e2e fallback: lane-change intent"
    ...
    if not force_release:
      ...
      if not valid_lane_geometry:
        ...
        if not full_lane_ready:
          ...
          # y = ax^2 + bx + c. Lane shape plus reduced heading/offset terms.
          ...
          lane_curvature = (lane_geometry_curvature +
                            LANE_LOCK_HEADING_GAIN * b / lookahead +
                            LANE_LOCK_OFFSET_GAIN * c / (lookahead * lookahead))
          ...
          if not valid_plan or not np.isfinite(lane_curvature):
            ...
            if path_disagreement > LANE_LOCK_MAX_PATH_DISAGREEMENT:
              _lane_lock_full_active = False
              fallback_reason = "stock-e2e fallback: extreme path disagreement"
            else:
