...
void update_leads(UIState *s, const cereal::RadarState::Reader &radar_state, const cereal::XYZTData::Reader &line) {
  for (int i = 0; i < 2; ++i) {
    ...
    if (lead_data.getStatus()) {
      ...
      calib_frame_to_full_frame(s, lead_data.getDRel(), -lead_data.getYRel(), z + 1.64, &s->scene.lead_vertices[i]);
    }
  }
}
...
void update_model(UIState *s,
  ...
  update_line_data(s, plan_position, 0.9, 1.64, &scene.track_vertices, max_idx, false);
}
